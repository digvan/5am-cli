#!/usr/bin/env python3
"""Render local IEDL examples and build HTML and GitHub Markdown galleries. No paid calls."""
import argparse
...
def build_markdown(cases, results):
    """A static GitHub view using existing pixels, with no JS or extra renders."""
    def escape(value):
        return html.escape(str(value)).replace('|', '&#124;').replace('[', '&#91;').replace(']', '&#93;')

    groups = list(dict.fromkeys(case['group'] for case in cases))
    rendered = sum(results.get(c['id'], {}).get('status') == 'Rendered' for c in cases if not c['remote'])
    lines = ['# IEDL visual gallery', '',
             f'{len(cases)} examples · {rendered} local renders. This page renders directly on GitHub.', '',
    ...
    for number, group in enumerate(groups, 1):
        lines.append(f'- [{escape(group)}](#category-{number})')
    for number, group in enumerate(groups, 1):
        lines.extend(['', f'<a name="category-{number}"></a>', '', f'## {escape(group)}', ''])
        for case in (c for c in cases if c['group'] == group):
            result = results.get(case['id'], {})
            status = 'Recipe only' if case['remote'] else result.get('status', 'Not rendered')
            success = not case['remote'] and status == 'Rendered'
            lines.extend([f'### {escape(case["title"])}', '', f'**{escape(status)}** — {escape(case["note"])}', ''])
            if success:
                lines.extend(['| Original | Edited |', '| --- | --- |',
                              f'| ![Original](source.jpg) | ![Edited: {escape(case["title"])}]({result["image"]}) |', ''])
            else:
                lines.extend(['No edited image is available for this example.', ''])
            links = [f'[Recipe](recipes/{case["id"]}.iedl)']
            if success:
                links.extend([f'[Full image]({result["image"]})', f'[Resolved report](resolved/{case["id"]}.json)'])
                for key, label in [('mask', 'Mask'), ('bundle', 'Replay bundle')]:
                    if result.get(key): links.append(f'[{label}]({result[key]})')
            lines.extend([' · '.join(links), ''])
    lines.extend(['---', '', 'Generated by `build_gallery.py` alongside `index.html`. [Photo and license](README.md#photo-and-license).', ''])
